...
class ReSample(Base_Algo):
    ...
    def cal_x0(self, xt, t, at, at_next, y_0, noise='ddpm', classes=None):
        ...
        with torch.enable_grad():
            ...
            optimizer = torch.optim.AdamW([x0_t_hat], lr=5e-3)
            for k in range(300):
                optimizer.zero_grad()
                loss = torch.nn.MSELoss()(self.H_funcs.H(self.model.differentiable_decode_first_stage(x0_t_hat)), y_0)
                loss.backward()
                optimizer.step()
    ...
    def cal_x0_dps(self, xt, t, at, at_next, y_0, noise='ddpm', classes=None):
        ...
        error = y_0 - self.H_funcs.H(self.model.differentiable_decode_first_stage(x0_t))
        ...
        x0_t -= 0.2 * 1 / at_next.sqrt() * grad * self.lam / loss.sqrt()
        return x0_t, add_up
