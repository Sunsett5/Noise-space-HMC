...
class H_functions:
    ...
    def H_scaled_inv(self, vec, scale):
        # for PiGDM
        # scale for sigma_y / r_t
        temp = self.Ut(vec)
        singulars = self.singulars()
        rescaled_singulars = 1 / (singulars**2 + scale ** 2)
        temp[:, :rescaled_singulars.shape[0]] = temp[:, :rescaled_singulars.shape[0]] * rescaled_singulars
        # return self.add_zeros(temp)
        return temp

    def H_dmps_guidance(self, vec, y, at, sigma_y):
        # for DMPS
        temp = y - 1/at.sqrt() * self.H(vec)
        temp = self.Ut(temp)
        singulars = self.singulars()
        rescaled_singulars = 1 / ((1-at)/at * singulars**2 + sigma_y ** 2)
        if sigma_y == 0:
            rescaled_singulars[singulars==0] = 0
        temp[:, :rescaled_singulars.shape[0]] = temp[:, :rescaled_singulars.shape[0]] * rescaled_singulars
        temp[:, :singulars.shape[0]] = temp[:, :singulars.shape[0]] * singulars
        temp = self.V(self.add_zeros(temp)) / at.sqrt()
        return temp
...
class NonlinearBlurOperator(H_functions):
    ...
    def prepare_nonlinear_blur_model(self, opt_yml_path):
        ...
        blur_model.eval()
        blur_model.load_state_dict(torch.load(model_path, weights_only=True)) 
